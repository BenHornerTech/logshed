...
### Fixed
- **Mobile Viewport Auto-Zoom on Input Focus**: Added `maximum-scale=1.0` to the HTML viewport meta tag to prevent mobile browsers (such as iOS Safari) from automatically zooming into the page when focusing compact form inputs, and updated login and setup modal password fields to `text-base sm:text-sm`.
